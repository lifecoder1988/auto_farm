...
export function getPotatoFramesMeta() {
  ...
  const POTATO_SHEET_URL =
    typeof window.POTATO_SHEET_URL === "string" &&
    window.POTATO_SHEET_URL.length
  ...
  for (let i = 0; i < 4; i++) {
    frames.push(
      new PIXI.Texture(sheetTex, new PIXI.Rectangle(i * size, 0, size, size))
    );
  }
  potatoFramesMeta = { frames, size };

  if (!sheetTex.valid) {
    sheetTex.once("update", () => {
      potatoFramesMeta = null;
    });
  }
