<line-height=50%><size=40px>外部编辑器</size>
</line-height>
...
你可以在<u><link="persistent_data_path/Saves">存档文件夹</link></u>中找到这些文件。
...
<line-height=50%><size=32px>使用 VS Code</size>
</line-height>
Visual Studio Code 是《编程农场》推荐使用的代码编辑器。

你可以<u><link="https://code.visualstudio.com/download">点击此处</link></u>下载。

下载完成后，需要在 VS Code 中安装 Python 扩展。

安装完 Python 扩展后，在 VS Code 中打开存放你的 .py 文件的<u><link="persistent_data_path/Saves">文件夹</link></u>。必须打开整个文件夹，而不只是单个文件，否则 __builtins__.py 文件将不起作用。
